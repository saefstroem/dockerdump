...
pub fn browse_directory(current_dir: &Path, base_dir: &Path) -> Vec<PathBuf> {
    ...
    loop {
        ...
        let selection = Select::new()
            .with_prompt(format!("Browsing: {}", current_path.strip_prefix(base_dir).unwrap_or(&current_path).display()))
            .items(&display_items)
            .default(0)
            .interact()
            .unwrap();
        ...
        match selection {
            ...
            i if i == entries.len() + 1 => { // Done browsing
                break;
            }
            i if i == entries.len() + 2 => { // Select files in current directory
                let file_items: Vec<String> = entries.iter()
                    .map(|e| {
                        let path = e.path();
                        let metadata = get_file_metadata(&path);
                        let file_name=e.file_name();
            ...
            i => { // Navigate into directory or select file
